...
const command: GluegunCommand = {
    ...
    run: async toolbox => {
        ...
        const clone = async () => {
            ...
            await shell.exec(`GIT_SSH_COMMAND='ssh -i ${key} -o IdentitiesOnly=yes' git clone ${myBibleRepo} ${myBibleRepoDir} || GIT_SSH_COMMAND='ssh -i ${key} -o IdentitiesOnly=yes' git pull origin master`)
        ...
        try {
            registeredVersionsinIndex = filesystem.read(`${myBibleRepoDir}/registeredVersions.js`)
            registeredVersionsinIndex = JSON.parse(registeredVersionsinIndex)
        } catch (error) {
            filesystem.write(`${myBibleRepoDir}/registeredVersions.js`, JSON.stringify({}))
        }
        ...
        const getBibleVersions = async () => {
            return new Promise(async (resolve, reject) => {
                let [err, care] = await to(cloudscraper.get(`https://www.bible.com/json/bible/versions/swh?filter=`))
                    ;[err, care] = await to(cloudscraper.get(`https://www.bible.com/json/bible/versions/swh?filter=`))
        ...
        const getVersionBooks = async (version) => {
            return new Promise(async (resolve, reject) => {
                ...
                let [err, care] = await to(cloudscraper.get(`https://www.bible.com/json/bible/books/${versionId}?filter=`))
                    ;[err, care] = await to(cloudscraper.get(`https://www.bible.com/json/bible/books/${versionId}?filter=`))
                if (err) return reject(err)
                care = {
                    version,
        ...
        const saveList = async (list) => {
            ...
            return new Promise(async (resolve, reject) => {
                ...
                filesystem.write(`/tmp/kjv-swahili/json/bible/versions/swh/${versionId}.json`, JSON.stringify(list));
                resolve(care)
            })
        }
        const pushRepo = async () => {
            return new Promise(async (resolve, reject) => {
                let clone;
                clone = await shell.exec(`cd ${myBibleRepoDir} && git add . && git commit -m "update"`);
                clone = await shell.exec(`cd ${myBibleRepoDir} && GIT_SSH_COMMAND='ssh -i ${key} -o IdentitiesOnly=yes' git push origin master`)
                if (clone.stderr.length && clone.stderr.includes('fatal:')) {
                    throw clone.stderr.length
                }
                resolve(true)
        ...
        const filterSavedVersions = async (versions) => {
            const versionExists = async (version) => {
                return new Promise(async (resolve, reject) => {
                    return resolve(filesystem.isDirectory(`${myBibleRepoDir}/${version.version.id}`) ? false : version)
                })
            }
            return new Promise(async (resolve, reject) => {
            ...
            return new Promise(async (resolve, reject) => {
                ...
                const downLoadVersionChapters = async (book) => {
                    return new Promise(async (resolve1, reject2) => {
                        console.log(`https://www.bible.com/json/bible/books/${versionId}/${book.usfm}/chapters`)
                        let [err, care] = await to(cloudscraper.get(`https://www.bible.com/json/bible/books/${versionId}/${book.usfm}/chapters`));
                        ;[err, care] = await to(cloudscraper.get(`https://www.bible.com/json/bible/books/${versionId}/${book.usfm}/chapters`));
                        console.log(care, typeof care)

                        if (err) return reject(err);
                        filesystem.dir(`/tmp/kjv-swahili/json/bible/books/${versionId}/${book.usfm}`)
                        try {
                            care = JSON.parse(care).items
                            filesystem.write(`/tmp/kjv-swahili/json/bible/books/${versionId}/${book.usfm}/chapters.json`, JSON.stringify(care));
                        } catch (error) { }
                        resolve();
                    })
                }
        ...
        const downloadChapterText = async (chapter, version) => {
            return new Promise(async (resolve, reject) => {
                ...
                if (filesystem.exists(`/tmp/kjv-swahili/bible/${versionId}/${chapter.usfm}.${version.local_abbreviation}/index.html`)) {
        ...
        const downloadChapterGroupText = async (chapterGroup, version) => {
            return new Promise(async (resolve, reject) => {
                let promises = chapterGroup.map((chapter) => { downloadChapterText(chapter, version) });
                let [err, care] = await to(Promise.all(promises))
                if (err) reject(err)
                resolve(care)
            })
        ...
        const downloadBookText = async (bookDir, version) => {
            return new Promise(async (resolve, reject) => {
                ...
                let promises = chapterGroups.map((chapterGroup) => { downloadChapterGroupText(chapterGroup, version) });
                for await (let num of promises) {
                    num;
                }
                resolve();
        ...
        const downloadVersionText = async (version) => {
            return new Promise(async (resolve, reject) => {
                ...
                // let versionBooks = version.versionBooks
                let books = filesystem.subdirectories(`/tmp/kjv-swahili/json/bible/books/${versionId}/`)
                // console.log(books)
                let promises = books.map((bookDir) => { downloadBookText(bookDir, version.version) })
                for await (let num of promises) {
                    num;
                    // console.log(num);
                }
        ...
        const downloadPageText = async (page) => {
            return new Promise(async (resolve, reject) => {
                ...
                if(!registeredVersionsinIndex[id]){
                    let versionInfo:any = filesystem.read(`/tmp/kjv-swahili/json/bible/versions/swh/${id}.json`)
                    ...
                    registeredVersionsinIndex[id] = {local_title, id, local_abbreviation}
                    // let currentVersionInfo = {

                    // }
                    let linkHtml = ``
                    for(let i in registeredVersionsinIndex){
                        let {local_title, id, local_abbreviation} = registeredVersionsinIndex[i];
