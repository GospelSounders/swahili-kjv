...
const command: GluegunCommand = {
  ...
  run: async toolbox => {
    ...
    let bookName = toolbox.parameters.options.b,
    ...
    if (!books.includes(bookName))
      return toolbox.print.error(`Unknown book ${bookName}`)
    ...
    if (!chapters.includes(chapter.toString()))
      return toolbox.print.error(`Unknown chapter ${chapter} in ${bookName}`)
