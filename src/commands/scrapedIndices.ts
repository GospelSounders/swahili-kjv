...
const command: GluegunCommand = {
    ...
    run: async toolbox => {
        ...
        let booksDirs = filesystem.subdirectories(`/tmp/kjv-swahili/json/bible/books/`)
        ...
        const getVersionInfo = async (path_) => {
            return new Promise((resolve, reject) => {
                ...
                // console.log(version)
                let template = filesystem.read(`/tmp/kjv-swahili/index.version.template.html`)
                template = template.replace('{{versionInfo}}', `${versionInfo.local_title} (${versionInfo.local_abbreviation})`)
                let linkHtml = '';
