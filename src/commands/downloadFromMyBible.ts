...
const command: GluegunCommand = {
  ...
  run: async toolbox => {
    ...
    let // username = toolbox.parameters.options.u,
    ...
    */
    ...
    let getVersionBooks = async versionId => {
      return new Promise((resolve, reject) => {
        axios
          .get(`https://www.bible.com/json/bible/books/${versionId}?filter=`)
          .then(response => {
            response = response.data.items
    ...
    if (myBibleBooks.length > 66)
      myBibleBooks.splice(39, myBibleBooks.length - 66) // remove apocrypha
    ...
    let downBibleText = async options => {
      ...
      return new Promise((resolve, reject) => {
        ...
        filesystem.dir(
          `/usr/share/swahili-kjv/data/${versionInfo['local_abbreviation']}`
        )
    ...
    let downloadBookText = async book => {
      return new Promise(async (resolve, reject) => {
        ...
        chapters = chapters.map(
          chapter => [{ book, chapter, shortBookName }][0]
        )
