...
let interfacesi = new nodeSwordinterface()

const command: GluegunCommand = {
  name: 'download',
  description: 'download KJV bible',
  run: async toolbox => {
    if (!interfacesi.repositoryConfigExisting())
      interfacesi.updateRepositoryConfig()
...
1This letter is from Paul, a slave of God and an apostle of Jesus 
    ...
    try {
      interfacesi.getLocalModule('KJV')
    } catch (err) {
      console.log('installing module KJV')
      interfacesi.installModule('KJV')
    }
    try {
      interfacesi.getLocalModule(moduleName)
    } catch (err) {
      console.log('installing module')
      interfacesi.installModule(moduleName)
    }

    // let booksS = interfacesi.getBookList('Swahili');
    // console.log(booksS); process.exit();

    // Print some module information
    // let kjv = interfacesi.getLocalModule('KJV');

    let books = interfacesi.getBookList('KJV')
    let allBooks = [].concat(books)
    books = interfacesi.getBookList(moduleName)
    let verseCounts = interfacesi.getBibleChapterVerseCounts(moduleName)
    ...
    // console.log(care);
    // let bibleText = interfacesi.getBibleText('KJV');
    // console.log(bibleText)

    interfacesi.enableMarkup()
    // let bibleText = interfacesi.getBibleText('KJV');

    let formatContent = content => {
      content = content.replace(/&nbsp;/g, ' ')
      content = content.replace(
        /<div class="sword-markup sword-quote-jesus"\/>([^<]*)<\/q>/g,
        `{{$1}}`
      )
      content = content.replace(
        /<w class="strong:[A-Z0-9]*["]?[^>]*>([^<]*)<\/w>/g,
        `$1`
      )
      content = content.replace(
        /<transChange type="added">([^<]*)<\/transChange>/g,
        `{$1}`
      )
      content = content.replace(
        /<div class="sword-markup sword-note"  type="study"><catchWord>([^<]*)<\/catchWord>([^<]*)<rdg type="[a-z\-]*">([^<]*)<\/rdg><\/div>/g,
        `[$1$2$3]`
      )
      ...
      content = content.replace(/<rdg>([^<]*)<\/rdg>/g, `$1`)
      content = content.replace(
        /<div class="sword-markup sword-note"  type="study"><catchWord>([^<]*)<\/catchWord>([^<]*)<\/div>/g,
        `[$1$2]`
      )
      ...
      content = content.replace(
        /<inscription>([^<]*)<\/inscription>/g,
        `[[$1]]`
      ) //
      content = content.replace(
        /<div class="sword-markup sword-section-title" canonical="true" type="psalm">([^<]*)<\/div>/g,
        `{{{$1}}}`
      ) // psalm title
      // extra notes
      content = content.replace(
        /<div class="sword-markup sword-note"  type="study"><catchWord>[^<]*<\/catchWord>[^<]*<transChange[^>]*>[^<]*<\/transChange><\/div>/g,
        ``
      )
      content = content.replace(
        /<div class="sword-markup sword-section-title" type="chapter">[^>]*<\/div>/g,
        ``
      )
      content = content.replace(
        /<div class="sword-markup sword-note"  type="study">[^>]*<\/div>/g,
        ``
      )
      ...
      content = content.replace(
        /<div class="sword-markup sword-quote-jesus"\/><div[^>]*>([^<]*)<\/q>/g,
        `{{$1}}`
      )
    ...
    // let chapterText = interfacesi.getChapterText('KJV', 'Matt', 5);
    ...
    let saveVerse = async verseObjIn => {
      ...
      if (chapterWithPlaceHolder.toString().length < 2)
        chapterWithPlaceHolder = '0' + chapterWithPlaceHolder
      if (chapterWithPlaceHolder.toString().length < 3)
        chapterWithPlaceHolder = '0' + chapterWithPlaceHolder

      let verseWithPlaceHolder: any = verseNr
      if (verseWithPlaceHolder.toString().length < 2)
        verseWithPlaceHolder = '0' + verseWithPlaceHolder
      if (verseWithPlaceHolder.toString().length < 3)
        verseWithPlaceHolder = '0' + verseWithPlaceHolder
      ...
      if (positionWithPlaceHolder.toString().length < 2)
        positionWithPlaceHolder = '0' + positionWithPlaceHolder
      ...
      switch (dialect) {
        case 'sqlite': //sqlite
          ...
          while (++chapter < numChapters) {
            let chapterText = interfacesi.getChapterText(
              moduleName,
              book,
              chapter + 1
            )
        ...
        default:
          // maria
          let bookText = interfacesi.getBookText(moduleName, book)
